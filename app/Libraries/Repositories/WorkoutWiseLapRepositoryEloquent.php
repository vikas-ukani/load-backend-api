...
class WorkoutWiseLapRepositoryEloquent extends BaseRepository implements UsersRepository
{
    ...
    protected function commonFilterFn(&$value, $input)
    {
        ...
        /** week_wise_workout_id and week_wise_workout_ids wise filter */
        if (isset($input['week_wise_workout_id'])) {
            $value = $value->whereRaw("find_in_set(" . $input['week_wise_workout_id'] .  ",week_wise_workout_ids)");
            // $value = $value->where('week_wise_workout_id', $input['week_wise_workout_id']);
        }
        // if (isset($input['week_wise_workout_ids']) && count($input['week_wise_workout_ids']) > 0) {
        //     $value = $value->whereIn('week_wise_workout_id', $input['week_wise_workout_ids']);
        // }
