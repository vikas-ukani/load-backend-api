...
/**
 * Class FeedCommentsRepositoryEloquent.
 *
 * @package namespace App\Libraries\Repositories;
 */
class FeedCommentsRepositoryEloquent extends BaseRepository implements UsersRepository
{
    ...
    protected function commonFilterFn(&$value, $input)
    {
        ...
        if (isset($input['search'])) {
            $value = $this->customSearch($value, $input, ['feed_id', 'user_id', 'comment']);
        }
        ...
        if (isset($input['feed_id'])) {
            $value = $value->where('feed_id', $input['feed_id']);
        }
        if (isset($input['feed_ids']) && count($input['feed_ids']) > 0) {
            $value = $value->whereIn('feed_id', $input['feed_ids']);
        }
        ...
        if (isset($input['last_comment_id']) && $input['last_comment_id'] >= 1) {
            $value = $value->where('id', '<', $input['last_comment_id']);
        } /* else if (isset($input['last_comment_id']) && $input['last_comment_id'] == 1) {
            $value = $value->where('id', '<', $input['last_comment_id']);
        } */
    }
    ...
    protected function getCommonPaginationFilterFn(&$value, $input)
    {
        ...
        /** id wise pagination */
        if (isset($input['last_comment_id']) && $input['last_comment_id'] == 0) {
            $value = $value->skip(0);
        }
