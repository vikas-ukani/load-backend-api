...
class LibraryRepositoryEloquent extends BaseRepository implements UsersRepository
{
    ...
    protected function commonFilterFn(&$value, $input)
    {
        ...
        //  user_detail, region_detail, body_part_detail, mechanic_detail, targeted_muscle_detail, action_force_detail
        ...
        if (isset($input['region_id'])) {
            $value = $value->where('region_id', $input['region_id']);
        }
        if (isset($input['region_ids']) && is_array($input['region_ids']) && count($input['region_ids'])) {
            $value = $value->whereIn('region_id', $input['region_ids']);
        }
